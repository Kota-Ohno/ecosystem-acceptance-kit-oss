...
export async function bootstrapWorkspace({
  ...
  const gitEnvironment = safeGitEnvironment(sourceEnvironment);
  ...
  const total = 2 + repositoryNames.length;
  let position = 0;
  ...
  const doctor = await step("environment-doctor", () => diagnose({ manifest, network: false, execute: doctorExecute }));
  if (doctor.checks?.find((entry) => entry.name === "git")?.available !== true) {
    throw new Error("Bootstrap requires git; install git and run pnpm bootstrap again");
  }
...
async function ensureCheckout({ name, entry, workspace, command, gitCommand, environment }) {
  const destination = join(workspace, name);
  if (existsSync(destination)) {
    refuseUnsafeDirectory(destination, `${name} checkout`);
    refuseUnsafeDirectory(join(destination, ".git"), `${name} .git directory`);
    await assertNoCheckoutFilters({ path: destination, revision: "HEAD", command, gitCommand, environment });
  ...
  try {
    ...
    await runChecked(command, gitCommand, ["fetch", "--quiet", "--depth=1", "origin", entry.revision], { cwd: destination, environment });
...
function resolveToolExecutable(name, pathValue, workspace) {
  ...
  for (const directory of String(pathValue).split(delimiter)) {
    for (const name of names) {
      ...
      try {
        ...
        if (resolved === workspace || resolved.startsWith(`${workspace}${sep}`)) continue;
        return resolved;
      } catch {
