...
export async function runAcceptance({ manifest, root, outputRoot, workspaceRoot, keepWorkspace = false }) {
  ...
  try {
    ...
    await step("agent-black-box:check", "npm", ["run", "check"], { cwd: checkouts.agentBlackBox });
    await step("sol-ledger:install", "npm", ["ci", "--ignore-scripts", "--no-audit", "--no-fund"], { cwd: checkouts.solLedger });
    await step("sol-ledger:test", "npm", ["test"], { cwd: checkouts.solLedger });
