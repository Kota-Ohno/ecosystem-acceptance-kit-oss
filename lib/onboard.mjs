...
export function formatOnboard(report) {
  ...
  const verificationCommand = formatVerificationCommand(report.workspace, report.directory, evidence.packetSha256);
  return [
    "Verified Evidence: READY",
    `  Workflow: ${localFile ? "caller-selected local file" : "local tutorial"}`,
    `  Evidence directory: ${terminalSafe(report.directory)}`,
    ...
    `  Packet: ${terminalSafe(evidence.artifacts.packet)}`,
    ...
    ...(verificationCommand
      ? ["  Re-verify later (copy and run):", `    ${verificationCommand}`]
      : ["  Re-verify later: rename workspace/output paths containing terminal controls, then run pnpm verify-evidence."]),
  ].join("\n");
}

function formatVerificationCommand(workspace, directory, packetSha256) {
  const packageRootArgument = shellQuote(packageRoot);
  const workspaceArgument = shellQuote(workspace);
  const directoryArgument = shellQuote(directory);
  if (!packageRootArgument || !workspaceArgument || !directoryArgument ||
      !/^[0-9a-f]{64}$/u.test(packetSha256 ?? "")) return undefined;
  return `pnpm --dir ${packageRootArgument} verify-evidence --workspace-root ${workspaceArgument} --directory ${directoryArgument} --expected-sha256 ${packetSha256}`;
}

function shellQuote(value) {
  if (typeof value !== "string" || [...value].some((character) => isUnsafeTerminalCodePoint(character.codePointAt(0)))) {
    return undefined;
  }
...
function isUnsafeTerminalCodePoint(codePoint) {
