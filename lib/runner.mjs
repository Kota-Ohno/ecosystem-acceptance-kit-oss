...
export function pnpmScriptArguments(script, arguments_ = []) {
  return [script, ...arguments_];
}
...
export async function runAcceptance({ manifest, root, outputRoot, workspaceRoot, keepWorkspace = false }) {
  ...
  try {
    ...
    await step("sol-ledger-contract:install", "npm", ["ci", "--ignore-scripts"], { cwd: checkouts.solLedgerContract });
    await step("agent-black-box:protocol-compatibility", "pnpm", pnpmScriptArguments("check:protocol", [checkouts.solLedgerContract]), { cwd: checkouts.agentBlackBox });
    ...
    await step("evidence-forge:sol-ledger-compatibility", "pnpm", pnpmScriptArguments("compatibility:sol-ledger", [checkouts.solLedgerContract]), { cwd: checkouts.evidenceForge });
    const stackOutput = join(output, "stack");
    await step("ecosystem:packed-acceptance", "pnpm", pnpmScriptArguments("acceptance:packed", ["--agent-black-box", checkouts.agentBlackBox, "--sol-ledger", checkouts.solLedger, "--output", stackOutput]), { cwd: checkouts.evidenceForge });
