...
async function executeCommand(command, arguments_, { cwd } = {}) {
  return await new Promise((resolvePromise) => {
    ...
    child.once("error", (error) => { clearTimeout(timer); resolvePromise({ ok: false, stdout, stderr: error.code ?? error.message }); });
    child.once("close", (code) => { clearTimeout(timer); resolvePromise({ ok: code === 0, stdout, stderr }); });
  });
}
