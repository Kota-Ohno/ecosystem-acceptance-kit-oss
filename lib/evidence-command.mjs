...
export function evidenceProgressReporter(write, {
  onEvidenceCreated = () => {}, interactive = false,
  ...
  let frame = 0;
  const stopAnimation = () => {
    if (animation !== undefined) cancel(animation);
    animation = undefined;
  };
  return ({ state, position, total, name, durationMs }) => {
    ...
    if (interactive) {
      const prefix = `[${String(position).padStart(2, "0")}/${String(total).padStart(2, "0")}]`;
      const label = labels[name] ?? name;
      if (state === "start") {
        stopAnimation();
        frame = 0;
        const render = () => {
          write(`\r\u001b[2K${prefix} ${frames[frame % frames.length]} ${label}…`);
          frame += 1;
        };
        render();
        animation = schedule(render, 80);
        animation?.unref?.();
        return;
      }
      stopAnimation();
      write(`\r\u001b[2K${prefix} ${marker} ${label}${duration}\n`);
      return;
    }
