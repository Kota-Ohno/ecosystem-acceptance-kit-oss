import {
  closeSync, constants, existsSync, fstatSync, lstatSync, mkdtempSync, openSync, readFileSync, realpathSync, rmSync,
  writeFileSync,
} from "node:fs";
...
export async function onboardFirstEvidence({
  ...
  const localFileRequested = source !== undefined || exact !== undefined || availableAt !== undefined || promoteImmediately;
  if (localFileRequested && (typeof source !== "string" || !source || typeof exact !== "string" || !exact ||
      typeof availableAt !== "string" || !availableAt || promoteImmediately !== true)) {
    throw new Error("Local-file onboarding requires --source, --exact, --available-at, and --promote-immediately together");
  }
  ...
  let fixedSourcePath;
  try {
    fixedSourcePath = sourcePath ? snapshotSource(sourcePath, sourceSnapshotRoot) : undefined;
  } catch (error) {
  ...
  try {
    ...
    evidence = await step(bootstrapSteps + 3, localFileRequested ? "evidence-forge:local-file" : "evidence-forge:first-evidence", async () => {
      ...
      const arguments_ = localFileRequested
        ? [
            "--silent", "forge", "--source", fixedSourcePath, "--exact", exact,
...
function snapshotSource(sourcePath, executionRoot) {
  let descriptor;
  try {
    descriptor = openSync(sourcePath, constants.O_RDONLY | constants.O_NOFOLLOW);
    const metadata = fstatSync(descriptor);
    if (!metadata.isFile() || metadata.size > 16 * 1024 * 1024) throw new Error();
    const snapshot = join(executionRoot, "caller-source.bin");
    writeFileSync(snapshot, readFileSync(descriptor), { mode: 0o600, flag: "wx" });
    return snapshot;
  } catch {
