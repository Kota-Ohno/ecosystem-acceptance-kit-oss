...
export function auditSecrets() {
  ...
  return {
    version: 1,
    kind: "EcosystemAcceptanceSecretAudit",
    outcome: "verified",
...
if (isMain) {
  try {
    ...
    process.stderr.write(`${JSON.stringify({ version: 1, kind: "EcosystemAcceptanceSecretAuditError", outcome: "error", code: "SECRET_AUDIT_FAILED", message: error instanceof Error ? error.message : "Secret audit failed" })}\n`);
    process.exitCode = 1;
  }
}
